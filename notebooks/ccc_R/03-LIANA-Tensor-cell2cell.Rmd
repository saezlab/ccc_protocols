...
## Introduction
Tensor decomposition as proposed in the [Tensor-cell2cell](https://www.nature.com/articles/s41467-022-31369-2#ref-CR5) paper, 
...
Note that in this case, we don't apply the `expr_prop` threshold and we keep
the values of all interactions as they are. Alternatively, one could be more
restrictive with the communication events thought to be occurring, and increase
`expr_prop` to a higher value, and `return_all` interactions with those below
`expr_prop` being assigned to the worst inferred value. That value can also
be manually replaced with e.g 0.
...
Optimal rank estimation can be computationally demanding, but required
to ensure robust results, one could consider increasing the `runs` parameter.
See below an elbow plot example, calculated if rank is set to NULL.
...
Similarly, one could consider how to fill missing values across the samples,
namely missing interactions cell types via the `lr_fill` and `cell_fill`
parameters, respecitvely. These are set to NaN by default and are thus imputed.
For example, setting them to 0 would suggest that missing values are
...
Upon Rank estimation, an optimal rank will be estimated and returned according
to the elbow of the normalized reconstruction error.

This error measures how different is the original tensor from the sum of R rank-1
tensors used to approximate it. We want to pick a rank that provides a good
trade-off that minimizes the number of factors (ranks) and the error.
...
The functions above can work with any other pathway or GO geneset.
So, let's also look at KEGG pathways.
```{r, fig.width=12, fig.height=8, message=FALSE, warning=FALSE}
# import KEGG Pathways from OmniPath
kegg <- OmnipathR::import_omnipath_annotations(resource="MSigDB", wide=TRUE) %>%
  # ^ this one has all of the GOs and other pathway databases in MSigDB :)
  dplyr::filter(collection=="kegg_pathways") %>%
    select(source="geneset", target="genesymbol") %>%
  # truncate Reactome names
    mutate(source = str_sub(source, start = 6, end = 30)) %>%
  mutate(weight=1)

# Generate LR resource
kegg_lr <- generate_lr_geneset(sce,
                               resource = kegg)

# Run ORA with decoupleR 
res <- decoupleR:::run_fgsea(mat = mat,
                             network = kegg_lr,
                             .source = "set",
                             .target = "lr",
                             minsize=10) %>%
...
```{r, fig.width=12, fig.height=8, message=FALSE, warning=FALSE}
# keep top 20 pathways for visualization
top20 <- res %>% 
  arrange(p_value) %>% 
  select(source) %>%
  distinct() %>% 
